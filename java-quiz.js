...
async function fetchQuestions() {
    try {
        const response = await fetch('http://localhost:8080/api/questions'); // API

        if(!response.ok){
            throw new Error('Network response was not ok');
        }

        questions = await response.json(); // Assume API returns an array of questions object
        ...
        document.getElementById('question-text').textContext = "Failed to load questions. Please try again.";
    }
}
...
function checkAnswer(selectedOption){
    ...
    if(selectedOption === currentQuestion.correctAnswer){
        correctAnswers++; // Incremmet score if the answer is correct
    }
...
function nextQuestion() {
    if(currentQuestionIndex < questions.length -1){
        ...
        localStorage.setItem('totalQuestions', totalQuestions); // Save toal questions
